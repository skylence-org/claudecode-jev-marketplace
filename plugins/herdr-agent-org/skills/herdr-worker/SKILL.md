...
- NO COMPOSER: you never type into another pane or agent, and never into the operator's live input (L11). Two channels only: `relay_send` for anything that must be on the record, and `SendMessage` (the native session ping, CC >= 2.1.224) as the wake. A ping carries a pointer, never content: board + relay carry the content, and pinging is never a license to sub-delegate or to steer a peer's lane.

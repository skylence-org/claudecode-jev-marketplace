#!/bin/bash
# jev.test.sh: regression tests for the jev helper and both jev-core hooks.
#
...
# silently.
#
# Run: bash plugins/jev-core/tests/jev.test.sh
set -uo pipefail
...
echo
echo "jev-core: $PASS passed, $FAIL failed"
[ "$FAIL" = 0 ] || exit 1
